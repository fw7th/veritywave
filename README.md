...
The core idea is that real cameras and AI generators produce images through different processes. A camera captures light physically; imperfect, noisy, optically constrained. A diffusion model or GAN synthesizes pixels mathematically, and that process leaves measurable fingerprints that are often invisible to the human eye but detectable through signal analysis.
...
**Domain specificity:** The model was trained on CIFAR-10 style images (natural scenes, animals, vehicles). Performance on other domains e.g. portraits, art, medical images; is untested and may degrade.
...
**Video assumption:** Video prediction aggregates per-frame predictions via mean probability. This assumes temporal consistency i.e. a video where only some frames are AI-generated will produce a blended confidence score rather than flagging the specific frames.
